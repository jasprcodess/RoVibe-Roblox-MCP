...
fn get_message(successes: String) -> String {
    format!("RoVibe Roblox MCP is ready to go.
...
fn get_claude_config() -> Result<PathBuf> {
    let config_path = if cfg!(target_os = "macos") {
        let home_dir =
            env::var_os("HOME").ok_or_else(|| eyre!("Could not find HOME directory"))?;
        Path::new(&home_dir)
...
fn get_cursor_config() -> Result<PathBuf> {
    let home_dir = env::var_os("HOME")
        ...
        .ok_or_else(|| eyre!("Could not find HOME or USERPROFILE directory"))?;
...
pub fn suggest_to_config_claude_code(exe_path: &Path) -> Result<String> {
    ...
    if config_path.exists() {
        Ok(format!("To add the MCP to Claude Code CLI run:\nclaude mcp add --transport stdio RoVibe_Studio -- '{}' --stdio", exe_path.display()))
    } else {
...
pub fn install_to_config(
    ...
    config["mcpServers"]["RoVibe_Studio"] = json!({
      "command": &exe_path,
      "args": [
        "--stdio"
      ]
...
pub async fn install() -> Result<()> {
    ...
    if let Err(e) = install_internal().await {
        tracing::error!("Failed initialize RoVibe Roblox MCP: {:#}", e);
    }
...
pub async fn install() -> Result<()> {
    ...
    let _ = alert_builder.set_title("RoVibe Roblox MCP").alert().show();
    Ok(())
}
